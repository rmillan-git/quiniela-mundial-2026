...
def sync_results_from_api(db: Session) -> dict:
    ...
    for m_api in matches_data:
        ...
        home_score_raw = ft.get("home")
        away_score_raw = ft.get("away")
        if home_score_raw is None or away_score_raw is None:
            continue

        # football-data.org adds penalty-shootout goals to fullTime.
        # e.g. 1-1 (90min) + 2-4 (pens) is stored as fullTime=3-5.
        # Subtract penalties to get the real match score (used for points).
        pens = score_obj.get("penalties") or {}
        pen_home = pens.get("home")   # None if no shootout
        pen_away = pens.get("away")
        home_score = home_score_raw - (pen_home or 0)
        away_score = away_score_raw - (pen_away or 0)

        # home_score_final = real match score (same value; 90-min vs ET distinction
        # requires reliable extraTime semantics — stored identically for now)
        ...
        # Auto-set winner_id for penalty-shootout matches
        new_winner_id = db_match.winner_id
        if pen_home is not None and home_score == away_score:
            api_winner = score_obj.get("winner")
            if api_winner == "HOME_TEAM":
                new_winner_id = db_match.home_team_id
            elif api_winner == "AWAY_TEAM":
                new_winner_id = db_match.away_team_id
        ...
        if (db_match.is_finished and db_match.home_score == home_score
                ...
                and db_match.winner_id == new_winner_id):
        ...
        db_match.winner_id = new_winner_id
